...
/    
    ...
    -- Cuando se intenta insertar un nuevo registro en la tabla "Prestamos", se debe verificar que el libro correspondiente está en estado prestado
CREATE OR REPLACE TRIGGER TR_Prestamos_PrestadoOno
BEFORE INSERT ON Prestamos
FOR EACH ROW
DECLARE
  vEstadoEjemplar Ejemplares.estado%TYPE;
BEGIN
  SELECT estado INTO vEstadoEjemplar
    FROM Ejemplares
    WHERE id = :NEW.idEjemplar;
  
  IF vEstadoEjemplar = 'Prestado' THEN
